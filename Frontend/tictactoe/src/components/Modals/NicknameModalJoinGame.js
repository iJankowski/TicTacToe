    ...
    handleJoiningGame = () => {
        this.props.setIsNickNameSet(()=> {
            if (this.props.nickName !== "" && this.props.isNickNameSet === true) {
                createAndJoin(this.props.nickName, "joinGame", this.state.gameCode)
                    .then(x => {
                            console.log(x.data)
                ...
                <div className="rounded-2xl p-10 bg-slate-900 relative">
                    ...
                    <div>
                        ...
                        {this.props.isNickNameSet === false ?
                            <input className="mt-2 bg-slate-800 block" onChange={this.props.setNickName}
                                   value={this.props.nickName}/> : null}
                        <div>What is game code?</div>
                        <input className="mt-2 bg-slate-800 block" onChange={this.setGameCode}
                               value={this.state.gameCode}/>
                        <div className="flex justify-end mt-2 buttonHover buttonSchema"
                             onClick={this.handleJoiningGame}>Submit
                        </div>
                    </div>
                </div>
            </div>
